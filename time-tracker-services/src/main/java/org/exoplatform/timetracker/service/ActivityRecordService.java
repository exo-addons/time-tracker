...
public class ActivityRecordService {
	...
	public ActivityRecordService(ActivityRecordStorage activityRecordstorage,
			TimeTrackerSettingsService timeTrackerSettingsService, TeamService teamService) {
	 ...
	 *                             occurs while creating ActivityRecord or its
	 *                             attached image
	 */
	...
	public ActivityRecord updateActivityRecord(ActivityRecord activityRecord, String username) throws Exception {
		...
		if (storedActivityRecord == null) {
			throw new IllegalArgumentException("ActivityRecord with id " + ActivityRecordId + " wasn't found");
		}
	 ...
	 *                                                   found
	 * @throws java.lang.IllegalAccessException          if user is not allowed to
	 *                                                   delete ActivityRecord
	 */
	public void deleteActivityRecord(Long activityRecordId, String username)
	 ...
	 */
	public RecordsAccessList getActivityRecordsList(String search, String activity, String type, String subType,
			String activityCode, String subActivityCode, String client, String project, String feature, String fromDate,
			String toDate, String userName, String location, String office, int offset, int limit, String sortBy,
			boolean sortDesc) {
		return activityRecordstorage.getActivityRecordsList(search, activity, type, subType, activityCode,
				subActivityCode, client, project, feature, fromDate, toDate, userName, location, office, offset, limit,
				sortBy, sortDesc);
	}
	 ...
	 * @param office          a {@link java.lang.String} object.
	 */
	public long countActivityRecords(String search, String activity, String type, String subType, String activityCode,
			String subActivityCode, String client, String project, String feature, String fromDate, String toDate,
			String userName, String location, String office) {
		return activityRecordstorage.countActivityRecords(search, activity, type, subType, activityCode,
				subActivityCode, client, project, feature, fromDate, toDate, userName, location, office);
	}
	...
	public String generateTSCode(List<Team> teams, ActivityRecord record, String exportType) {
		...
		tsCode = String
				.valueOf(record.getActivityTime().toInstant().atZone(ZoneId.systemDefault()).toLocalDate().getYear());
		...
		if (exportType.equals("fr")) {
			...
			if (team.equals("Architects") || team.equals("Designers") || team.equals("Management")
					|| team.equals("Analysts") || team.equals("ITOP")) {
				if (type.equals("SERV")
						|| (subType.equals("FIXP") || subType.equals("CONS") || subType.equals("HOST"))) {
	...
	String getProject(ActivityRecord record) {
		...
		if (record.getActivity() != null && record.getActivity().getProject() != null
				&& StringUtils.isNotEmpty(record.getActivity().getProject().getCode())
				&& !record.getActivity().getProject().getCode().equals("<EXO>")) {
	...
	String getClient(ActivityRecord record) {
		...
		if (record.getActivity() != null && record.getActivity().getProject() != null
				...
				&& StringUtils.isNotEmpty(record.getActivity().getProject().getClient().getCode())) {
	...
	String getActivity(ActivityRecord record) {
		...
		if (record.getActivity() != null && record.getActivity().getActivityCode() != null
				&& StringUtils.isNotEmpty(record.getActivity().getActivityCode().getCode())) {
	...
	String geSubActivity(ActivityRecord record) {
		...
		if (record.getActivity() != null && record.getActivity().getSubActivityCode() != null
				&& StringUtils.isNotEmpty(record.getActivity().getSubActivityCode().getCode())
				&& !record.getActivity().getSubActivityCode().getCode().equals("<FEAT>")) {
			subActivity = "_" + record.getActivity().getSubActivityCode().getCode();
		}
		return subActivity;
	}

	public List<ActivityRecord> getUserActivityRecords(String search, String activity, String type, String subType,
			String activityCode, String subActivityCode, String client, String project, String feature, String fromDate,
			String toDate, String userName, String location, String office, String sortBy, Boolean sortDesc,
			Boolean export, String exportType) {

		RecordsAccessList recordsAccessList = getActivityRecordsList(search, activity, type, subType, activityCode,
				subActivityCode, client, project, feature, fromDate, toDate, userName, location, office, 0, 0, sortBy,
				sortDesc);
		IdentityManager identityManager = CommonsUtils.getService(IdentityManager.class);

		if (StringUtils.isNotEmpty(search) || StringUtils.isNotEmpty(activity) || StringUtils.isNotEmpty(type)
				|| StringUtils.isNotEmpty(subType) || StringUtils.isNotEmpty(activityCode)
				|| StringUtils.isNotEmpty(subActivityCode) || StringUtils.isNotEmpty(client)
				|| StringUtils.isNotEmpty(project) || StringUtils.isNotEmpty(feature)
				|| (StringUtils.isNotEmpty(userName) && userName.equals("all"))) {
			...
			for (ActivityRecord activityRecord : activityRecordList) {
				...
				if (export) {
					try {
						activityRecord.setTsCode(generateTSCode(teamService.getTeamsList(activityRecord.getUserName()),
								activityRecord, exportType));
					} catch (Exception e) {
		...
		try {
			if (StringUtils.isNotEmpty(fromDate)) {
				...
				String from = recordsAccessList.getActivityRecords().get(recordsAccessList.getSize().intValue() - 1)
						.getActivityDate();
			...
			for (LocalDate d : getDatesBetween(from_, to_)) {
				...
				act = recordsAccessList.getActivityRecords().stream().filter(c -> c.getActivityDate().equals(day))
				...
				if (act.size() > 0) {
					...
					for (ActivityRecord activityRecord : act) {
						...
						if (export) {
							activityRecord
									.setTsCode(generateTSCode(teamService.getTeamsList(activityRecord.getUserName()),
											activityRecord, exportType));
						}
					...
					if (dayOfWeek.getValue() == 6 || dayOfWeek.getValue() == 7) {
						ActivityRecord weekEndRecord = new ActivityRecord(null, userName, day, actDate, "Week End", "",
								office_, null, "", null, weekEndActivity, null, null,
								identityManager.getOrCreateUserIdentity(userName).getProfile().getFullName(), null);
						if (export) {
							weekEndRecord.setTsCode(
									generateTSCode(teamService.getTeamsList(userName), weekEndRecord, exportType));
						}
						activityRecordList.add(weekEndRecord);
					} else {
						activityRecordList.add(new ActivityRecord(null, userName, day, actDate, "", "", "", null, "",
								null, null, null, null,
								identityManager.getOrCreateUserIdentity(userName).getProfile().getFullName(), null));
					}
				}
	...
	private List<ActivityRecord> groupRedendantActivities(List<ActivityRecord> activities) {
		...
		for (ActivityRecord activityRecord : activities) {
			String actCode = activityRecord.getActivityDate() + "_" + activityRecord.getDescription() + "_"
					+ activityRecord.getActivity().getCode() + "_" + activityRecord.getActivity().getProject().getCode()
					+ "_" + activityRecord.getActivity().getProject().getClient().getCode();
